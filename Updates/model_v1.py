...
class Summarization_Model:
    def __init__(self):
        ...
        self.summary_tokenizer = T5Tokenizer.from_pretrained(self.summary_checkpoint)
        self.summary_base_model = T5ForConditionalGeneration.from_pretrained(self.summary_checkpoint, device_map='auto', torch_dtype=torch.float32)
        self.summary_pipe = pipeline(
            'summarization',
            model=self.summary_base_model,
            tokenizer=self.summary_tokenizer,
            max_length=1000,
            min_length=50)

    def summarize(self, text):
        # Use the summarization model to generate a summary
        summary = self.summary_pipe(text)
