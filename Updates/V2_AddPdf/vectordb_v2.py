...
class VectorStore:
    ...
    def pdf_to_vectors(self, pdf_path):
        # Read PDF
        ...
        # Preprocess Text (this is a simple example, can be more complex)
        text = text.replace('\n', ' ').strip()

        # Convert to Vectors using the EmbeddingRetriever
        return self.retriever.embed_queries(queries=[text]).tolist()[0]

    def add_pdf_to_db(self, pdf_path):
        try:
            vector = self.pdf_to_vectors(pdf_path)
            # Assuming you want to store the entire PDF content as a single document
            doc_to_index = {'content': "PDF: " + os.path.basename(pdf_path), 'meta': {'vector': vector}}
            self.document_store.write_documents([doc_to_index])
            logging.info("PDF added to database successfully.")
        except Exception as e:
            logging.error("Error during adding PDF to database: %s", str(e))
